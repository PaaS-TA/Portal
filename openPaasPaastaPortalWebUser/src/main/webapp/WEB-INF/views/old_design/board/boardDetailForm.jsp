...
<div id="boardViewArea">
    ...
    <div class="col-sm-12 pt10">
        ...
        <c:choose>
            <c:when test="${insertFlag eq checkCudC}">
                <h4 class="modify_h4"> 커뮤니티 새 글 등록 </h4>
            </c:when>
            <c:when test="${insertFlag eq checkCudU}">
                <h4 class="modify_h4"> 커뮤니티 글 상세 </h4>
            </c:when>
            <c:when test="${insertFlag eq checkCudR}">
                <h4 class="modify_h4"> 커뮤니티 답글 등록 </h4>
            </c:when>
        </c:choose>
    </div>
    ...
    <div class="panel content-box col-sm-12 col-md-12">
        <form id="boardForm" name="boardForm" class="form-horizontal" role="form">
            ...
            <c:choose>
                <c:when test="${insertFlag eq checkCudU}">
                    <div class="form-group">
                        <label class="control-label col-sm-1 custom_text_align custom_control_label" >  </label>
                        <div class="col-sm-1">
                        ...
                        <div class="col-sm-4" align="right">
                            <span style="font-weight: bold; color: gray; font-size: large; display: none;" id="beforeEdit" > <span id="boardModify" style="cursor: pointer; ">수정</span> | <span id="boardDelete"  style="cursor: pointer; ">삭제</span> </span>
                            <span style="font-weight: bold; color: gray; font-size: large; display: none;" id="afterEdit" > <span id="modifySave" style="cursor: pointer; ">저장</span> | <span id="modifyCancel" style="cursor: pointer; ">취소</span> </span>
                        </div>
                    </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-11">
                    <input type="text" class="form-control modify_form_control toCheckString" id="title" name="title" maxlength="100" style="background: white; cursor: text" onkeypress="if(event.keyCode=='13') return false;">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-11">
                    <textarea class="form-control modify_form_control" rows="10" id="content" name="content" style="background: white; cursor: text"></textarea>
                </div>
            </div>
        ...
        <div class="form-group">
            <div class="col-sm-12 pd0" align="right">
                <div class="">
                    <c:choose>
                        <c:when test="${insertFlag eq checkCudU}">
                            <button type="button" class="btn btn-default" id="btnReply">답변쓰기</button>
                        ...
                        <c:otherwise>
                            ...
                            <button type="button" class="btn btn-save" id="btnList">취소</button>
                        </c:otherwise>
                    </c:choose>
                </div>
            </div>
        ...
        <c:choose>
            <c:when test="${insertFlag eq checkCudU}">
                ...
                <div id="boardCommentViewArea">
                    <div class="form-group custom-search-form">
                        <div class="col-sm-12 pt10">
                            <label class="control-label col-sm-1 custom_text_align custom_control_label" > 댓글 </label>
                            <span class="glyphicon glyphicon-chevron-down" id="commentDown" style="cursor: pointer;"></span>
                            <span class="glyphicon glyphicon-chevron-up" id="commentUp" style="cursor: pointer;"></span>
                        </div>
                    </div>
                        ...
                        <form id="commentForm" name="commentForm" class="form-horizontal" role="form">
                            ...
                            <div id="commentArea">
                                ...
                                <div class="col-sm-11" style="padding-right: 10px;">
                                    <textarea class="form-control modify_form_control" rows="3" id="commentContent" name="content" style="background: white; cursor: text;  "></textarea>
                                </div>
                                <div class="col-sm-1" style="padding-left: 0px;">
                                    <button type="button" class="btn btn-default" id="btnNewCommentSave" style=" height: 85px; ">댓글등록</button>
                                </div>
                            </div>
                        </form>
                    </div>
    ...
    </div> <%-- div "form-group custom-search-form" END --%>
        ...
        for (var i = 0; i < getList.length; i++) {
            ...
            if (getList[i] != undefined) {
                ...
                $("#boardCommentList").append(
                        ...
                        +"<td  style='width: 12%; border-left: none; border-right: none; '>"
                               + "<span id='commentEditArea"+i+"' >"
                               + "<span style='cursor: pointer;' onclick='procCommentModify("+ getList[i].no +",\""+ getList[i].content + "\")'>수정</span>" + " | " + "<span  style='cursor: pointer;' onclick='procGetCommentReplyNum("+ getList[i].no +")'>삭제</span>"
                               + "<br>"
                        + "</span>"
                        + "<span id='replySpan' style='cursor: pointer;' onclick='commentReply("+ getList[i].no +","+ getList[i].parentNo + "," + getList[i].groupNo + ")'>댓글</span>"
                        + " </td>"
...
//        var reqTitle = $('#title');
    ...
    function getReplyNum() {
        ...
        procCallAjax(GET_REPLY_NUM_PROC_URL, param, procCallbackGetReplyNum);
    }
    var procCallbackGetReplyNum = function (data) {
        procCallSpinner(SPINNER_SPIN_STOP);
        var replyNum = data.info.replyNum;
...
//    // BIND :: BUTTON EVENT
    ...
    function procCommentDelete(data) {
        ...
        if (replyNum > 0) { // level == depth, 0: 원글, 1~: 답글
            ...
            $("#modalCancelBtn").show();
             procPopup('댓글', DELETE_MESSAGE, 'procCallSpinner(SPINNER_SPIN_START); procCallAjax(DELETE_BOARD_COMMENT_PROC_URL, {no : $("#boardCommentNo").val()}, procCallbackCommentDelete);');
