 ...
 * }
        ...
        for (var CN, cidx = 0, __arr = mobileView.column; (cidx < __arr.length && (CN = __arr[cidx])); cidx++) {
            if (CN.display == true || CN.display == undefined) {
                ...
                if (mobileView.label != false) {
                    tpo.push("<span class='label1'>" + CN.label + "</span>");
    ...
    getEditorFormValue: function (form, dataSet, value, key, CH, idAttr) {
        ...
        if (form.type == "hidden") {
            ...
            result.push("<div style=\"padding-right:" + paddingRight + ";\">");
            result.push("<input type=\"text\" id=\"" + formID + "\" name=\"" + key + "\" value=\"" + getFormValue(form.value, dataSet[key]) + "\" class=\"AXInput" + formClass + "\" style=\"" + formWidth + formHeight + formStyle + "\" maxlength='30' />");
            result.push("</div>");
        }
    ...
    getEditorBody: function (dataSet, isfix) {
        ...
        for (var r = 0; r < cfg.editor.rows.length; r++) {
            ...
            if (r == 0 && isfix == undefined) {
                tpo.push("<td class=\"bodyNullTd11\" id=\"" + cfg.targetID + "_AX_editornull\" rowspan=\"" + cfg.editor.rows.length + "\"><div class=\"tdRelBlock\">&nbsp;</div></td>");
            }
            tpo.push("</tr>");
        }
        return tpo.join('');
